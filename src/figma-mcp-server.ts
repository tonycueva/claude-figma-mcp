// figma-mcp-server.ts
import { MCPServer as Server, MCPCapability as Capability, MCPResource as Resource, MCPFunction as Function } from './lib/mcp-types';
import * as WebSocketModule from 'ws';
import { figmaConfig } from './figma-config';
/**
 * Figma MCP Server
 * 
 * This server implements the Model Context Protocol to allow Claude
 * to interact with Figma for creating wireframes and prototypes.
 * 
 * It works in conjunction with a Figma plugin that uses the Figma Plugin API
 * to actually create the design elements.
 */

// Create a custom type that combines the server and client WebSocket types
type WebSocketConnection = WebSocketModule.WebSocket & {
...
const wss = new WebSocketModule.Server({ port: figmaConfig.websocket.port });
console.log(`WebSocket server started on port ${figmaConfig.websocket.port}`);
let figmaConnection: WebSocketConnection | null = null;

wss.on('connection', (ws: WebSocketConnection) => {
  console.log('Figma plugin connected');
  figmaConnection = ws;
  
  ws.on('message', (message: WebSocketModule.Data) => {
    try {
...
const responseHandlers: { [key: string]: (response: any) => void } = {};
...
// Frame Management
const frameResource = new Resource({
  id: 'frame',
  name: 'Figma Frame',
  description: 'Manage frames and artboards in Figma',
});

frameResource.addFunction(
  new Function({
    id: 'create-frame',
    name: 'Create Frame',
...
const elementsResource = new Resource({
  id: 'elements',
  name: 'UI Elements',
  description: 'Create and manage UI elements in Figma',
});

elementsResource.addFunction(
  new Function({
    id: 'create-rectangle',
...
// Layout Management
const layoutResource = new Resource({
  id: 'layout',
  name: 'Layout',
  description: 'Create common layout patterns in Figma',
});

layoutResource.addFunction(
  new Function({
    id: 'create-layout',
    name: 'Create Layout',
...
const prototypeResource = new Resource({
  id: 'prototype',
  name: 'Prototype',
  description: 'Create and manage interactive prototypes',
});

prototypeResource.addFunction(
  new Function({
...
// Export Management
const exportResource = new Resource({
  id: 'export',
  name: 'Export',
  description: 'Export designs from Figma',
});

exportResource.addFunction(
  new Function({
    id: 'export-frame',
    name: 'Export Frame',
...
const figmaCapability = new Capability({
...
mcpServer.listen(figmaConfig.mcpServer.port).then(() => {
  console.log(`Figma MCP Server started on port ${figmaConfig.mcpServer.port}`);
}).catch((error) => {
