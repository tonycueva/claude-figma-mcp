...
export class FigmaMcpServer {
  ...
  private figmaApiClient: FigmaApiClient | null = null;
  private useFigmaApiDirectly: boolean = false;
  ...
  constructor(config: ServerConfig) {
    ...
    if (config.figmaApiKey) {
      try {
        this.figmaApiClient = new FigmaApiClient(config.figmaApiKey);
        Logger.log("Figma API client initialized");
      } catch (error) {
        Logger.error("Failed to initialize Figma API client:", error);
      }
    }
    ...
    return new Promise((resolve, reject) => {
      if (!this.figmaConnection) {
        ...
        if (this.figmaApiClient) {
          Logger.log("No WebSocket connection to Figma plugin, using Figma API directly");
  ...
  private async executeFigmaApiCommand(command: string, params: any): Promise<any> {
    if (!this.figmaApiClient) {
    ...
    Logger.log(`Executing Figma API command: ${command}`);
    ...
    switch (command) {
      ...
      case 'exportFrame':
        ...
        const result = await this.figmaApiClient.exportImage(
          params.fileKey,
          params.frameId,
          params.format?.toLowerCase() as any || 'png',
          params.scale || 1
        );
