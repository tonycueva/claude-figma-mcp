...
const figma_api_client_1 = require("./figma-api-client");
exports.Logger = {
    log: (...args) => {
        console.log(...args);
    },
...
class FigmaMcpServer {
    constructor(config) {
        ...
        this.figmaApiClient = null;
        this.useFigmaApiDirectly = false;
        this.responseHandlers = {};
        this.config = config;
        // Initialize Figma API client if API key is available
        if (config.figmaApiKey) {
            try {
                this.figmaApiClient = new figma_api_client_1.FigmaApiClient(config.figmaApiKey);
                exports.Logger.log("Figma API client initialized");
            }
            catch (error) {
                exports.Logger.error("Failed to initialize Figma API client:", error);
            }
        }
    ...
    async sendToFigma(command, params) {
        // If we're using the Figma API directly and have a client, use it instead of WebSocket
        if (this.useFigmaApiDirectly && this.figmaApiClient) {
            return this.executeFigmaApiCommand(command, params);
        }
        return new Promise((resolve, reject) => {
            if (!this.figmaConnection) {
                // If no WebSocket connection but we have a Figma API client, try to use it as fallback
                if (this.figmaApiClient) {
                    exports.Logger.log("No WebSocket connection to Figma plugin, using Figma API directly");
                    this.useFigmaApiDirectly = true;
                    this.executeFigmaApiCommand(command, params)
                        .then(resolve)
                        .catch(reject);
                    return;
                }
                reject(new Error('No active connection to Figma plugin and no Figma API fallback available'));
                return;
            }
    ...
    async executeFigmaApiCommand(command, params) {
        if (!this.figmaApiClient) {
            throw new Error('Figma API client not initialized');
        }
        // This is a simplified implementation - in a real application, you'd need to implement
        // more complex logic to map between MCP commands and Figma API calls
        exports.Logger.log(`Executing Figma API command: ${command}`);
        switch (command) {
            case 'createProject':
                // Figma API doesn't directly support creating projects
                // This is just a placeholder response
                return {
                    projectId: `api-project-${Date.now()}`,
                    name: params.name
                };
            case 'createFrame':
                // Figma API doesn't allow direct frame creation
                // In a real implementation, you might use the Figma Plugin API feature
                // that allows executing plugin code in Figma
                return {
                    frameId: `api-frame-${Date.now()}`,
                    width: params.width,
                    height: params.height
                };
            case 'exportFrame':
                // This one we can actually implement with the API if we have a file key
                if (!params.fileKey) {
                    throw new Error('File key is required for exporting frames with Figma API');
                }
                const result = await this.figmaApiClient.exportImage(params.fileKey, params.frameId, params.format?.toLowerCase() || 'png', params.scale || 1);
                return {
                    exportUrl: result.images[params.frameId]
                };
            default:
                throw new Error(`Command '${command}' not supported when using Figma API directly`);
        }
    }
